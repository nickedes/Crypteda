def sbox(i, ltable, atable):
    s = gmul(i, ltable, atable)
    x = gmul(i, ltable, atable)
    s = int(s, 16)
    x = int(x, 16)
    for c in range(4):
        s = (s << 1) | (s >> 7)
...
def gmul(i, ltable, atable):
    if i == 0:
        return 0
    else:
        return atable[(255 - int(ltable[i],16))]
...
def schedule(key, iteration,atable,ltable):
    ...
    for k in range(0, len(new_key)):
        i = int(new_key[k], 16)
        new_key[k] = sbox(i,ltable,atable)
    print(new_key)
